...
class FanPixelCard extends HTMLElement {
  ...
  static async getConfigElement() { return document.createElement("fan-pixel-card-editor"); }
  static getStubConfig() { return { entity: "", name: "风扇" }; }
  ...
  _render() {
    this.shadowRoot.innerHTML = `
      ...
      <div class="card">
        <div class="head">
          <div class="icon" data-icon><ha-icon icon="mdi:fan-off"></ha-icon></div>
          <div class="name">${this._config.name || "风扇"}</div>
    ...
    this.shadowRoot.querySelector(".card").addEventListener("click", () => {
      this._hass.callService("fan", "toggle", { entity_id: this._config.entity });
    });
    this._update();
  }
  ...
  _update() {
    ...
    const color = on ? "#4cde8b" : "#9aa0a6";
    const icon = this.shadowRoot.querySelector("[data-icon]");
    icon.style.background = on ? "rgba(76,222,139,.18)" : "rgba(255,255,255,.08)";
    icon.innerHTML = `<ha-icon icon="${on ? "mdi:fan" : "mdi:fan-off"}" style="color:${color}"></ha-icon>`;
    const status = this.shadowRoot.querySelector("[data-status]");
    status.textContent = on ? "运行中" : "已关闭";
    status.style.color = color;
    status.style.background = on ? "rgba(76,222,139,.15)" : "rgba(255,255,255,.07)";
    const pct = on ? (parseFloat(st.attributes.percentage) || 0) : 0;
    this.shadowRoot.querySelector("[data-value]").innerHTML = on ? `${Math.round(pct)}<span class="unit">% 速度</span>` : "关闭";
  }
}
...
class FanPixelCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    picker.hass = this._hass; picker.value = this._config.entity || ""; picker.includeDomains = ["fan"];
...
if (!customElements.get("fan-pixel-card")) customElements.define("fan-pixel-card", FanPixelCard);
if (!window.customCards.some((c) => c.type === "fan-pixel-card")) {
  window.customCards.push({ type: "fan-pixel-card", name: "风扇卡片", description: "简洁风风扇卡：速度大字，点击开关，支持可视化编辑", preview: true });
}
console.info(`%c FAN-CARD %c v${CARD_VERSION} `, "color:#fff;background:#2b6ff2;font-weight:700", "color:#2b6ff2;background:#fff");
