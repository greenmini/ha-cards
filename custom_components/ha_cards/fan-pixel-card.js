/**
 * Fan Card · PIXEL EDITION v1.0.0
 * Nothing 点阵像素风风扇/风机卡片 —— 统一设计语言，支持 HA 可视化编辑。
 *
 * 用法：
 *   type: custom:fan-pixel-card
 *   entity: fan.xiaomi_cn_xxx          # 必填
 *   name: 空气净化器
 */
...
class FanPixelCard extends HTMLElement {
  constructor() {
    super();
  ...
  setConfig(config) {
    ...
    if (!this._config.entity) throw new Error("fan-pixel-card: 需要配置 entity");
    this._render();
  }
  ...
  static async getConfigElement() {
    return document.createElement("fan-pixel-card-editor");
  }

  static getStubConfig() {
    return { entity: "", name: "风扇" };
  }
  ...
  _render() {
    this.shadowRoot.innerHTML = `
      ...
      <div class="card">
        <div class="top reveal" style="--i:0">
          <span class="sq"></span>
          <span class="ttl">FAN // ${this._config.name}</span>
        ...
        <div class="metrics">
          <div class="m reveal" style="--i:3" data-m="speed">
            <div class="num"><canvas></canvas></div>
            <div class="vu">${"<span class='seg'></span>".repeat(SEGS)}</div>
            <div class="lb">SPEED</div><div class="un">%</div>
          </div>
        </div>
    ...
    this._speedEls = { cv: this.shadowRoot.querySelector('[data-m="speed"] canvas'), segs: this.shadowRoot.querySelectorAll('[data-m="speed"] .seg') };
  ...
  _update() {
    ...
    if (this._statusKey !== (on ? "ON" : "OFF")) {
      ...
      cn.innerHTML = `<ha-icon icon="${on ? "mdi:fan" : "mdi:fan-off"}"></ha-icon><span>${on ? "运行中" : "已关闭"}</span>`;
    ...
    const pct = on ? (parseFloat(st.attributes.percentage) || 0) : 0;
    drawPixels(this._heroCv, on ? Math.round(pct).toString() : "OFF", on ? 7 : 6, color);
    const prev = this._values.pct;
    if (prev === undefined) drawPixels(this._speedEls.cv, Math.round(pct).toString(), 4, color);
    else if (Math.abs(prev - pct) > 0.5) {
      if (this._speedEls.cv._raf) cancelAnimationFrame(this._speedEls.cv._raf);
      const start = performance.now();
      const step = (now) => {
        const t = Math.min((now - start) / 650, 1);
        const e = 1 - Math.pow(1 - t, 3);
        drawPixels(this._speedEls.cv, Math.round(prev + (pct - prev) * e).toString(), 4, color);
        if (t < 1) this._speedEls.cv._raf = requestAnimationFrame(step);
      };
      this._speedEls.cv._raf = requestAnimationFrame(step);
    }
    this._values.pct = pct;
    this._setVu(this._speedEls.segs, on ? pct / 100 : 0, color);

    this.shadowRoot.querySelectorAll(".blk").forEach((b, i) => {
      const litB = on && i < Math.ceil((pct / 100) * 4);
  ...
  _toggle() {
    ...
    if (!eid) return;
    this._hass.callService("fan", "toggle", { entity_id: eid });
  }
...
class FanPixelCardEditor extends HTMLElement {
...
customElements.define("fan-pixel-card-editor", FanPixelCardEditor);
window.customCards = window.customCards || [];
if (!customElements.get("fan-pixel-card")) {
  customElements.define("fan-pixel-card", FanPixelCard);
}
if (!window.customCards.some((c) => c.type === "fan-pixel-card")) {
  window.customCards.push({
    type: "fan-pixel-card",
    name: "风扇卡片 · 像素版",
    description: "Nothing 点阵像素风风扇卡：速度点阵 + VU 电平，支持可视化编辑",
    preview: true,
  });
}
console.info(`%c FAN-PIXEL-CARD %c v${CARD_VERSION} `, "color:#0d0d0d;background:#e04b34;font-weight:700", "color:#e04b34;background:#0d0d0d");
